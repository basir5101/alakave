...
export default function ByVintage({ userId = "" }: { userId: string }) {
  ...
  return (
    <div className="xl:px-16 xl:mx-20 mt-10 mx-5">
      ...
      <div className="flex flex-wrap border-b-2">
        {[
          ...
          <button
            className={`${
              item === vintage ? "bg-accent text-white" : ""
            } px-4 py-1 rounded-t-lg `}
            key={item}
            onClick={() => setVintage(item || "")}
          >
            {item}{" "}
          </button>
        ))}
